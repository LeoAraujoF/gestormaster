...
CREATE OR REPLACE FUNCTION public.prepare_alert_history_retry(p_alert_history_id uuid, p_user_id uuid, p_allow_pending boolean DEFAULT false, p_retry_job_id text DEFAULT NULL::text)
 RETURNS text
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public'
...
BEGIN
  ...
  IF v_history.contact_reservation_id IS NOT NULL THEN
    UPDATE public.contact_reservations
    SET status = 'reserved', decision_reason = 'MANUAL_RETRY_QUEUED', updated_at = now()
    WHERE id = v_history.contact_reservation_id;
  END IF;
...
REVOKE ALL ON FUNCTION public.prepare_alert_history_retry(p_alert_history_id uuid, p_user_id uuid, p_allow_pending boolean, p_retry_job_id text) FROM PUBLIC, anon, authenticated, service_role;

GRANT EXECUTE ON FUNCTION public.prepare_alert_history_retry(p_alert_history_id uuid, p_user_id uuid, p_allow_pending boolean, p_retry_job_id text) TO service_role;
