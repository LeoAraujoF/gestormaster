-- Bloqueia reserva de contato automática para clientes que pediram opt-out do
-- WhatsApp (whatsapp_opt_out = true). O campo já existia (migration
-- 20260817120000_whatsapp_safety_controls.sql) mas nunca era checado antes de
-- um disparo automático de cobrança/renovação/campanha — só silenciava o menu
-- do bot conversacional. Categoria 'manual' fica de fora de propósito: já tem
-- seu próprio fluxo de confirmação humana (allow_manual_override).
...
BEGIN
  ...
  SELECT c.whatsapp_opt_out INTO v_opt_out
  ...
  IF v_opt_out AND p_category <> 'manual' THEN
    RETURN QUERY SELECT NULL::uuid, 'blocked'::text, 'WHATSAPP_OPT_OUT'::text, NULL::text, NULL::date;
    RETURN;
  END IF;
