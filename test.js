...
async function test() {
  ...
  const { error: e1 } = await supabase
    ...
    .limit(1)
  console.log('Error 1:', e1 ? e1.message : 'Success')
  ...
  console.log('Testing query 3: users!referrer_id(full_name)')
  const { error: e3 } = await supabase
    .from('affiliate_earnings')
    .select('*, referred_user:users!referrer_id(full_name)')
    .limit(1)
  console.log('Error 3:', e3 ? e3.message : 'Success')
}
